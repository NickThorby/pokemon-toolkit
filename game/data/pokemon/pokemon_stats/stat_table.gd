...
func _init(
    ...
    elif p_stat_data.has(GameData.PokemonStat.HP):
        self.hp = p_stat_data[GameData.PokemonStat.HP]
...
func get_stat(p_stat):
    if typeof(p_stat) == TYPE_STRING:
        ...
        if p_stat == GameData.PokemonStat.HP:
            return hp
        elif p_stat == GameData.PokemonStat.ATTACK:
            return attack
